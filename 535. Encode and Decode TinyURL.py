...
class Codec:
    url_mapper = {}
    CONSTANT = 'http://tinyurl.com/'

    def encode(self, longUrl: str) -> str:
        """Encodes a URL to a shortened URL.
        """
        hash_value = hash(longUrl)
        Codec.url_mapper[str(hash_value)] = longUrl

        return Codec.CONSTANT + str(hash_value)

    def decode(self, shortUrl: str) -> str:
        """Decodes a shortened URL to its original URL.
        """

        map_key = shortUrl.split('/')[-1]

        return Codec.url_mapper[map_key]
