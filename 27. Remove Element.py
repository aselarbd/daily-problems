...
class Solution:
    def removeElement(self, nums: list[int], val: int) -> int:
        ...
        first_removal_index = -1
        is_removal_happened = False
        removal_count = 0

        for i in range(len(nums)):
            if nums[i] == val:
                if not is_removal_happened:
                    first_removal_index = i
                    is_removal_happened = True
                nums[i] = '_'
                removal_count += 1
        ...
        removal_index = first_removal_index

        for i in range(first_removal_index, len(nums)):
            if nums[i] != '_':
                nums[removal_index] = nums[i]
                nums[i] = '_'
                removal_index += 1
